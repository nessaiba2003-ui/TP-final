...
public class PerformanceReport {
    ...
    private void testPerformance(String testName, Supplier<?> testFunction) {
        ...
        // Récupération des statistiques Hibernate
        Session session = emf.createEntityManager().unwrap(Session.class);
        Statistics stats = session.getSessionFactory().getStatistics();

        // nous allons Enregistrer les résultats
    ...
    private void generateReport() {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {
            ...
            if (needsCacheOptimization) {
                ...
                writer.println("   - Ajustation des paramètres de cache pour les entités fréquemment accédées");
            ...
            if (needsQueryOptimization) {
                ...
                writer.println("   - Utilisation de JOIN FETCH pour éviter les problèmes N+1");
                writer.println("   - Créer des index sur les colonnes fréquemment utilisées dans les WHERE!");
                writer.println("   - Optimisation des requêtes qui prennent plus de 500ms");
            }

            writer.println("\n3. Des Recommandations générales:");
            writer.println("   - Surveillance régulière des performances avec des outils comme JProfiler");
            ...
            System.out.println("Okay le Rapport est généré: " + fileName);

        } catch (IOException e) {
            System.err.println("Attention erreur lors de la génération du rapport: " + e.getMessage());
            e.printStackTrace();
        }
    }
